    ...
    for i, csv_filename in enumerate(csv_files, start=1):
        ...
        try:
            with open(input_path, mode='r', newline='', encoding='utf-8') as f:
                ...
                # csv.reader should already strip the quotes, but to be safe:
                ...
                for row in reader:
                    if len(row) <= date_index:
                        # Row doesn't have enough columns (data might be malformed)
                        continue
                ...
                # --------------------------------------------------------------
                if min_date_found <= EARLIEST_REQUIRED_DATE and max_date_found >= LATEST_REQUIRED_DATE:
