...
def download_csv_files(filename, output_dir):
    ...
    # Randomize the order of files to download
    ...
    # Loop through each CSV filename in a random order
    for i, csv_filename in enumerate(csv_filenames, 1):
        ...
        if progress % 1 == 0:
            print(f"Progress: Downloaded {csv_filename}, {i} of {total_files} files ({progress}%)")
