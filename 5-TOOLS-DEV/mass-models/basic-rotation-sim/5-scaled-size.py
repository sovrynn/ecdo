...
# Principal moments of inertia (about x, y, z body axes)
I1, I2, I3 = 2.0, 1.0, 10.0  # Example values

# Initial angular velocity in the body frame (rad/s)
omega0_body = np.array([1.0, 0.5, 0.8])
...
# --------------------------------------------------
# 5) Compute box dimensions from inertia I1,I2,I3
#    Assume a uniform rectangular prism of mass M=1
# --------------------------------------------------
M = 1.0  # (you can change if desired)
A = 12 * I1 / M
B = 12 * I2 / M
C = 12 * I3 / M

# Solve for half-dimensions lx, ly, lz
lx = np.sqrt((B + C - A)/2)
ly = np.sqrt((A + C - B)/2)
lz = np.sqrt((A + B - C)/2)

# Build the corners in BODY frame
...
# Principal axes in body frame (x, y, z)
body_axes = np.eye(3)  # columns = [1,0,0], [0,1,0], [0,0,1]
...
ax.set_ylabel('Y')
ax.set_zlabel('Z')
ax.set_title('Rotation of a 3D Rigid Body (Box Sized by I1, I2, I3)')
