...
def parse_markdown(markdown_text, styles, params):
    ...
    while i < len(lines):
        ...
        if header_match:
            ...
            style_name = f"CustomHeading{header_level}"
            style = False
        ...
        image_footer_match = re.match(r"\*\*Exhibit .*", line) or line[:2] == "``"
        if image_footer_match:
            if line[:2] == "``":
                line = line[2:]
        ...
        #             list_item = ListItem(Paragraph(item_text, styles["Custom"]))
        ...
        #     para = Paragraph(para_text, styles["Custom"])
        #     elements.append(para)
        #     i += 1
        ...
        if citations == False:
            ...
            elements.append(Paragraph(line, styles["Custom"]))
        i += 1
...
def register_font(font_name):
    ...
    if os.path.exists(font_file):
        ...
        italic_font_file = f"{font_name}-Italic.ttf"
        if os.path.exists(bold_font_file):
            pdfmetrics.registerFont(TTFont(f"{font_name}-Bold", bold_font_file))
            pdfmetrics.registerFont(TTFont(f"{font_name}-Italic", italic_font_file))
            pdfmetrics.registerFontFamily(font_name,normal=font_name,bold=f"{font_name}-Bold",italic=f"{font_name}-Italic",boldItalic=f"{font_name}-Bold")
            print("Registered bold and italic font family.")
        else:
...
def generate_pdf(md_file_path, output_file_path, params):
    ...
    for level in range(smallest_header_level, 0, -1):
        ...
        # give a slight boost to the largest header size
        if level == 1: font_size += size_increment * 0.4
