...
Unfortunately seems reportlab is incapable of flowing text around images: https://groups.google.com/g/sphinx-users/c/DPIDBN3p9tU?pli=1

Other options are reStructuredText and Sphinx but seem they also don't support it.

## Prompt creation v2

Write a python script that takes an input directory along with other formatting parameters read from an input file, and for each Markdown file (.md) in the input directory, generates a properly formatted PDF, which also embeds images that are embedded in the Markdown files. Use pylatex.
...
- font, string (ttf font file in the same directory, optionally bold and italic font files as well)
...
- Optionally, in the Markdown, there will be a width value, from 0 to 1, on the same line as the image embedding, following it, separated by a space. If this is found, then scale the image size to that portion of the remaining width inside the image margins (ex: 0.5 = 50%).
- Optionally, in the Markdown, after the scale value, there will be a number 1, also separated by a space. If this is the case, the image should not be centered, and should not have any margins, but should instead be right-aligned with the corresponding scale width, and the rest of the text should FLOW AROUND IT, starting with the PARAGRAPH/TEXT IMMEDIATELY PRECEDING THE IMAGE.
...
- If the image preceding an image footer is to be right-aligned (denoted by the number 1 following the scale value) then instead of centering the image footer with margins, include it in the same right-aligned image block as the image, without margins.
