...
def main():
    ...
    for dt_tag in dt_tags:
        ...
        if next_sib and next_sib.name == 'dd':
            # Preserve the raw HTML content within <dd> (minus the <dd> tag itself)
            dd_content = next_sib.decode_contents()

            # Construct the output file path
            output_filename = f"{file_id}.txt"
            output_path = os.path.join(output_dir, output_filename)

            # Write the raw HTML inside <dd> to the output file (saved as UTF-8)
