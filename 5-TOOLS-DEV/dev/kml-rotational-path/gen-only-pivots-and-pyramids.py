...
add_cross_to_kml(kml, (elat, elon), RED, 5, CROSS_THICK) #pivots
add_cross_to_kml(kml, (wlat, wlon), RED, 5, CROSS_THICK)
POINTER_SIZE = 2.4
if use_marker == "y":
    add_point_kml((x, y), kml, marker_size)

def add_kml_marker(kml, lat, lon, name="Marker", icon_url=None, color="ff0000ff", scale=1.0):
    """
    Adds a marker to an existing KML object.
    
    Parameters:
    - kml (simplekml.Kml): The existing KML object.
    - lat (float): Latitude of the marker.
    - lon (float): Longitude of the marker.
    - name (str): Name of the marker.
    - icon_url (str): URL of a custom icon (default: None, uses default pin).
    - color (str): Color of the marker in KML AABBGGRR format.
    - scale (float): Size scale of the icon.
    """
    pnt = kml.newpoint(name=name, coords=[(lon, lat)])
    
    # Style the marker
    pnt.style.iconstyle.color = color  # KML color format: AABBGGRR
    pnt.style.iconstyle.scale = scale  # Adjusts marker size
