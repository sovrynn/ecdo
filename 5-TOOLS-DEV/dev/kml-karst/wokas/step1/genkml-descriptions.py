...
OUTPUT_KML = "karst-aquifers-description.kml"
...
MARKER_ICON = "http://maps.google.com/mapfiles/kml/shapes/donut.png"
MARKER_SCALE = 1.5  # icon scale in KML
MARKER_SCALE_MEDIUM = 1.5  # icon scale in KML
MARKER_SCALE_SMALL = 1.5  # icon scale in KML
LABEL_SCALE = 1
...
def main():
    ...
    kml_lines.append(f"        <color>{COLOR_BLUE}</color>")
    kml_lines.append(f"        <scale>{MARKER_SCALE_SMALL}</scale>")
    ...
    kml_lines.append("      <IconStyle>")
    kml_lines.append(f"        <color>{COLOR_BLUE}</color>")
    kml_lines.append(f"        <scale>{MARKER_SCALE_MEDIUM}</scale>")
    kml_lines.append(f"        <Icon><href>{MARKER_ICON}</href></Icon>")
    kml_lines.append("      </IconStyle>")
    kml_lines.append("      <LabelStyle>")
    kml_lines.append(f"        <scale>{LABEL_SCALE}</scale>")
    ...
    for csv_file in csv_files:
        ...
        if D < THRESHOLD_WHITE:
            ...
            white_count += 1
            water_range = "0.01 - 0.1 m^3/s"
            size = 'Small'
        elif D < THRESHOLD_GRAY:
            style_id = "grayMarker"
            gray_count += 1
            water_range = "0.1 - 1 m^3/s"
            size = 'Large'
        else:
    ...
    <Placemark>
      <description>
        <![CDATA[
          <p><b>{size} Karst Aquifer</b> ({water_range})</p>
          <p>{A}</p>
          <p>Source: WoKaS https://www.nature.com/articles/s41597-019-0346-5</p>
        ]]>
      </description>
