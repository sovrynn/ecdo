...
def detect_encoding(file_path, num_bytes=10000):
    ...
    Returns:
        str: Detected encoding or 'cp1252' as default.
    """
    try:
        ...
        if encoding and confidence > 0.5:
            ...
            print(f"Warning: Low confidence ({confidence:.2f}) for encoding of file {file_path}. Defaulting to 'cp1252'.")
            return 'cp1252'
...
def validate_csv_file(file_path):
    ...
    try:
        with file_path.open('r', encoding=encoding, errors='strict') as file:
            lines = file.readlines()
    except UnicodeDecodeError as ude:
        print(f"UnicodeDecodeError reading file {file_path} with encoding {encoding}: {ude}")
        return False
    except Exception as e:
