...
def main():
    ...
    # Create the first 100 rows as (0,0,0)
    first_part = np.zeros((100, 3))

    # Create the last 400 rows as a linear progression from (0,0,0) to (-1,0,0)
    # np.linspace returns 400 values from 0 to -1 for the x-coordinate.
    x_values = np.linspace(0, -1, 400)
    # y and z remain 0 for these values
    second_part = np.column_stack((x_values, np.zeros(400), np.zeros(400)))

    # Combine both parts into a single array
    positions = np.vstack((first_part, second_part))
    
    # scene.draw_debug_spheres(poss=poss, radius=0.05, color=(0, 0, 1, 0.7))
    for i in range(500):
        # time.sleep(0.5)
        terrain.set_pos([positions[i],0,0])
        scene.step()

        cam.render()

    # stop recording and save video. If `filename` is not specified, a name will be auto-generated using the caller file name.
    cam.stop_recording(save_to_filename='pbd6.mp4', fps=60)
