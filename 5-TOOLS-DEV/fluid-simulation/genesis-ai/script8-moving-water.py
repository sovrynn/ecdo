...
def main():
    ...
    def create_scene(scene, height_field, water_level):
        ...
        for y, row in enumerate(height_field):
            for x, cell_value in enumerate(row):
                if cell_value < water_level:
                    ...
                    try:
                        liquid_entities.append(add_liquid_column(scene, lowx, lowy, lowz, height))
        ...
        return (terrain,liquid_entities)
    ...
    terrain,liquid_entities = create_scene(scene, height_field, WATER_LEVEL)
    ...
    for i in range(500):
        # time.sleep(0.5)
        if i == 100:
            for ent in liquid_entities:
                ent.set_velocity(gs.tensor([1,0,0], requires_grad=True))
    ...
    cam.stop_recording(save_to_filename='s8.mp4', fps=60)
