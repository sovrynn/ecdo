...
Description:
    ...
    • For each mapping:
        ...
        – Sends each chunk to ChatGPT if not already done, saving each chunk's response to second_word/x.txt
...
parser = argparse.ArgumentParser(
    description="Chunk a file and stream it through GPT, storing per-chunk outputs in folders."
)
...
for m_idx, pair in enumerate(mappings, start=1):
    ...
    out_path = folder / f"{second_word}.tex"
